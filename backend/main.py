...
MAX_INTERVIEW_HISTORY_MESSAGES = int(os.getenv("MOCK_INTERVIEW_HISTORY_LIMIT", "20"))
MAX_RESUME_FILE_SIZE_BYTES = int(
    os.getenv("MAX_RESUME_FILE_SIZE_BYTES", str(4 * 1024 * 1024))
)
...
def build_interview_messages(history: List[InterviewMessage], user_message: str) -> List[Any]:
    messages: List[Any] = [SystemMessage(content=SYSTEM_PROMPT)]

    for item in history[-MAX_INTERVIEW_HISTORY_MESSAGES:]:
...
def analyze_resume_bytes(pdf_bytes: bytes, job_description: str = "") -> Dict[str, Any]:
...
def analyze_resume_file(pdf_path: str, job_description: str = "") -> Dict[str, Any]:
    with open(pdf_path, "rb") as file_obj:
        return analyze_resume_bytes(file_obj.read(), job_description)
...
def health() -> Dict[str, Any]:
    return {
        ...
        "max_resume_file_size_mb": round(MAX_RESUME_FILE_SIZE_BYTES / (1024 * 1024), 2),
    }
...
def chat(req: ChatRequest) -> ChatResponse:
    ...
    conversation = build_interview_messages(req.history, req.message.strip())
    ...
    try:
        ...
        if len(file_bytes) > MAX_RESUME_FILE_SIZE_BYTES:
            raise HTTPException(
                status_code=413,
                detail=(
                    "Resume file is too large for this deployment. "
                    f"Please upload a PDF under {round(MAX_RESUME_FILE_SIZE_BYTES / (1024 * 1024), 1)} MB."
                ),
            )

        return analyze_resume_bytes(file_bytes, job_description)
